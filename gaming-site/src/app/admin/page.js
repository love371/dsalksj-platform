...
export default function AdminPage() {
  ...
  const handleInsertImageUrl = () => {
    ...
    insertHtmlAtCursor(`
      <div style="margin:18px 0;">
        <img src="${url}" alt="Inserted Media" style="width:100%;border-radius:16px;" />
      </div>
    `);
  };
  ...
  const uploadGalleryImage = async (file) => {
    try {
      ...
      if (!response.ok) {
        alert(data.message || "Gallery image upload failed");
        return;
      }
      ...
      alert("Server error while uploading gallery image");
    } finally {
